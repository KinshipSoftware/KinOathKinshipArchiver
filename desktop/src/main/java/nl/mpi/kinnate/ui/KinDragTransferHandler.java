...
//currentArbilNode.getUrlString()
    ...
    private boolean attachMetadata() {
        System.out.println("importMetadata");
        try {
            EntityDocument entityDocument = new EntityDocument(entityCollection.getProjectRecord().getFileInProject(targetEntity.getUniqueIdentifier()).toURI(), new ImportTranslator(true), sessionStorage, entityCollection.getProjectRecord());
